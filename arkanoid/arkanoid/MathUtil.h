...
struct Sweep
{
	...
	void GetTransform(Transform* xfb, float beta) const;
...
{
	return Mat22(MulT(A, B.ex), MulT(A, B.ey));
}
...
// Multiply a matrix times a vector.
inline Vec2 MulT22(const Mat33& A, const Vec2& v)
{
	return Vec2(A.ex.x * v.x + A.ey.x * v.y, A.ex.y * v.x + A.ey.y * v.y);
}
